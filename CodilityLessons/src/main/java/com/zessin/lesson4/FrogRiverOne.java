...
public class FrogRiverOne {
    ...
    public int solution(int numberOfLeaves, int[] array) {
        Map<Integer, Integer> positionsWithoutLeaves = new HashMap<>();
        int lastPositionRemoved = 0;
        int position = 0;

        for (int i = 1; i <= numberOfLeaves; i++) {
            positionsWithoutLeaves.put(i, i);
        }

        while (!positionsWithoutLeaves.isEmpty() && position < array.length) {
            lastPositionRemoved = position;
            positionsWithoutLeaves.remove(array[position++]);
        }

        return positionsWithoutLeaves.isEmpty() ? lastPositionRemoved : -1;
    }
